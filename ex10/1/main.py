...
if op=="Sign":
    ...
    k = int(input())
    # entl='{:016b}'.format(len(sm3.str_2_bin(id)))
    # tmp=sm3.str_2_bin(id)
    # print(entl,tmp)
    # t=pad(entl)+pad(tmp)+pad(bin(a)[2:])+pad(bin(b)[2:])+pad(bin(G[0])[2:])+pad(bin(G[1])[2:])+pad(bin(P[0])[2:])+pad(bin(P[1])[2:])
    # Z='{:064x}'.format(int(sm3.hash1(t),16))
    Z=get_Z(id,P,G,a,b)
