...
def SM4(s,op,K):
    if op==1:
        ...
        return decrypt(s,K)
def outprint(t,n):
    tmp=[0 for i in range(n//2)]
    for i in range(n//2):
        tmp[n//2-1-i]=(t>>8*i)&0xff
    for i in tmp:
...
for i in range(len(InputList)):
    ...
    list+=InputList[i]
tmp=list.split(" ")
while tmp[len(tmp)-1]==" " or tmp[len(tmp)-1]=="":
    del tmp[len(tmp)-1]
# print(tmp)
...
for i in data:
    l1=l1+str('{:02x}'.format(i))
P=[0 for i in range(len(l1)//n+1)]
for i in range(len(P)-1):
    temp=0
    for j in range(n):
        temp=(temp<<4)+int(l1[i*n+j],16)
    P[i]=temp
# print(len(l1)%n,l1)
i=len(l1)//n
temp=0
for j in range(len(l1)%n):
    temp=(temp<<4)+int(l1[i*n+j],16)
for j in range(len(l1)%n,n):
    temp=(temp<<4)
P[i]=temp
# for i in P:
#     print(hex(i))
...
for i in range(len(P)):
    ...
    if i!=(len(P)-1):
        ...
        # print(len(l1)%n//2)
        outprint2(c,n,len(l1)%n//2)
    if op==1:
        reg=((reg<<(n*4))+(temp2>>(128-n*4)))&0xffffffffffffffffffffffffffffffff
    else:
        reg = ((reg << (n * 4)) + temp2) & 0xffffffffffffffffffffffffffffffff
